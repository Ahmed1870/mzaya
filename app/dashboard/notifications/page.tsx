...
import { Bell, Trash2, CheckCircle, Clock, ArrowRight } from 'lucide-react'
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (authUser) {
        ...
        // Realtime update: تحديث الصفحة لو وصل إشعار جديد وأنت فاتحها
  ...
  const markAllRead = async () => {
    ...
    fetchNotifs(user.id)
    Swal.fire({ title: 'تم!', text: 'تم تحديد الكل كمقروء', icon: 'success', background: '#0a0a0a', color: '#fff', timer: 1500, showConfirmButton: false })
  }
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={markAllRead} style={{ background: '#111', border: '1px solid #222', color: '#ccc', padding: '10px 15px', borderRadius: '12px', fontSize: '13px', cursor: 'pointer' }}>
              تحديد الكل كمقروء
            </button>
            <button onClick={clearAll} style={{ background: 'rgba(255, 75, 43, 0.1)', border: '1px solid rgba(255, 75, 43, 0.3)', color: '#ff4b2b', padding: '10px 15px', borderRadius: '12px', fontSize: '13px', cursor: 'pointer' }}>
              مسح الكل
            </button>
          </div>
        </header>
          ...
          <div style={{ display: 'grid', gap: '15px' }}>
            {notifications.map((n) => (
              <div key={n.id} style={{
                ...
                borderRadius: '20px',
                border: '1px solid',
                borderColor: n.is_read ? '#1a1a1a' : '#d4af3733',
                display: 'flex',
                ...
                <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                  <div style={{ 
                    padding: '12px', 
                    background: n.type === 'wallet' ? 'rgba(30, 215, 96, 0.1)' : 'rgba(212, 175, 55, 0.1)',
                    borderRadius: '15px'
                  }}>
                    {n.type === 'wallet' ? <CheckCircle size={20} color="#1ed760" /> : <Bell size={20} color="#d4af37" />}
                  </div>
                  <div>
