...
import { Crown, Zap, CheckCircle2, MessageCircle, Star, ShieldCheck, Rocket } from 'lucide-react'
  ...
  useEffect(() => {
    async function init() {
      ...
      if (user) {
        ...
        setCurrentPlan(data?.plan_name || 'مجانية')

        // رادار الاستقبال اللحظي - Realtime Broadcast
    ...
    <div style={{ background: '#050505', color: '#fff', minHeight: '100vh', padding: '40px 20px', direction: 'rtl' }}>
      <div style={{ textAlign: 'center', marginBottom: '60px' }}>
        ...
        <div style={{ marginTop: '20px', background: 'rgba(212, 175, 55, 0.1)', padding: '12px 30px', borderRadius: '50px', border: '1px solid #d4af3733', display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
          ...
          باقتاك الحالية: <span style={{ color: '#d4af37', fontWeight: '900' }}>{currentPlan}</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
        ...
        {/* الباقة الاحترافية */}
        <div style={{ ...cardStyle, border: currentPlan === 'احترافية' ? '2px solid #d4af37' : '1px solid #1a1a1a', boxShadow: currentPlan === 'احترافية' ? '0 0 30px rgba(212, 175, 55, 0.2)' : 'none' }}>
          {currentPlan === 'احترافية' && <div style={activeBadge}>نشطة حالياً</div>}
          ...
          <button 
            onClick={() => handleProcess('احترافية', 99)} 
            disabled={loading || currentPlan === 'احترافية'} 
            style={{ ...btnGold, opacity: currentPlan === 'احترافية' ? 0.5 : 1 }}
          >
            {currentPlan === 'احترافية' ? 'مشترك بالفعل' : 'اشترك الآن'}
          </button>
        </div>

        {/* باقة البيزنس */}
        <div style={{ ...cardStyle, border: currentPlan === 'بيزنس' ? '2px solid #1ed760' : '1px solid #1a1a1a', boxShadow: currentPlan === 'بيزنس' ? '0 0 30px rgba(30, 215, 96, 0.2)' : 'none' }}>
          {currentPlan === 'بيزنس' && <div style={{ ...activeBadge, background: '#1ed760' }}>نشطة حالياً</div>}
          ...
          <button 
            onClick={() => handleProcess('بيزنس', 199)} 
            disabled={loading || currentPlan === 'بيزنس'} 
            style={{ ...btnGreen, opacity: currentPlan === 'بيزنس' ? 0.5 : 1 }}
          >
            {currentPlan === 'بيزنس' ? 'مشترك بالفعل' : 'تطوير العضوية'}
          </button>
        </div>
...
const btnGold = { 
  background: 'linear-gradient(45deg, #d4af37, #fbf5b7)', 
  color: '#000', 
  border: 'none', 
  padding: '18px', 
  width: '100%', 
...
const btnGreen = { 
  background: 'linear-gradient(45deg, #1ed760, #a2f9c1)', 
  color: '#000', 
  border: 'none', 
  padding: '18px', 
  width: '100%', 
