  ...
  const handleUpdate = async (e: any) => {
    ...
    try {
      ...
      const { error } = await supabase.from('products').update({
        ...
        stock_quantity: parseInt(form.stock), // مزامنة المخزن
        description: form.description,
        image_url: imageUrl
      }).eq('id', id)

      if (error) throw error
      await Swal.fire({ icon: 'success', title: 'تم التحديث بنجاح', background: '#111', color: '#fff' })
  ...
  return (
    <div className="animate-fade-up" style={{ color: 'white', padding: '1.2rem', maxWidth: '500px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gap: '1.2rem' }}>
        <div onClick={() => galleryRef.current?.click()} style={{ background: '#080808', height: '220px', borderRadius: '2rem', border: '2px dashed #222', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
          {imagePreview ? <img src={imagePreview} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <div style={{textAlign:'center', opacity: 0.4}}><Upload size={40} color="#D4AF37" /><p style={{fontSize:'0.8rem', marginTop:'12px'}}>تغيير الصورة</p></div>}
        </div>
        ...
        <div style={{ background: '#111', padding: '1.5rem', borderRadius: '2rem', border: '1px solid #222', display: 'grid', gap: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{background:'#050505', borderRadius:'15px', border:'1px solid #222', padding:'5px 12px'}}>
              <label style={{fontSize:'0.6rem', color:'#444', fontWeight:700}}>السعر الحالي</label>
            ...
            <div style={{background:'#050505', borderRadius:'15px', border:'1px solid #222', padding:'5px 12px'}}>
              <label style={{fontSize:'0.6rem', color:'#444', fontWeight:700}}>الكمية</label>
          ...
          <textarea style={{ width: '100%', height:'100px', background: '#050505', border: '1px solid #222', padding: '1rem', borderRadius: '15px', color: '#ccc' }} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />

          <button onClick={handleUpdate} disabled={saving} style={{ background: 'linear-gradient(45deg, #D4AF37, #FBF5B7)', color: '#000', padding: '1.2rem', borderRadius: '1.5rem', fontWeight: 950, border: 'none', marginTop: '0.5rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
            {saving ? <Loader2 className="animate-spin" /> : <><CheckCircle2 size={22} /> تحديث البيانات</>}
          </button>
        </div>
      </div>
