...
export default function NewProductPage() {
  ...
  useEffect(() => {
    async function check() {
      ...
      if (status) setStats({ plan: status.label, count: count || 0, max: status.maxLimit, unlimited: status.isUnlimited, canAI: status.canUseAI })
    }
    check()
  }, [])
  ...
  const handleSave = async () => {
    ...
    if (!stats.unlimited && stats.count >= stats.max) return Swal.fire('عفواً', `باقتك ${stats.plan} لا تسمح بأكثر من ${stats.max} منتجات`, 'error')
    ...
    const { error } = await supabase.from('products').insert({
      user_id: user?.id, name: form.name, price: parseFloat(form.price) || 0,
      stock: parseInt(form.stock) || 0, description: form.description, image_url: imageUrl
    })

    if (!error) {
      Swal.fire('تم!', 'أضيف المنتج بنجاح', 'success').then(() => router.push('/dashboard/products'))
    }
    setSaving(false)
  }
  ...
  return (
    <div style={{ background: '#050505', minHeight: '100vh', color: '#fff', padding: '20px', direction: 'rtl' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px' }}>
        ...
        <div style={{ background: '#d4af3722', padding: '8px 15px', borderRadius: '15px', color: '#d4af37', fontSize: '0.8rem', border: '1px solid #d4af3744' }}>
      ...
      <div style={{ maxWidth: '500px', margin: '0 auto', display: 'grid', gap: '20px' }}>
        <div onClick={() => document.getElementById('img')?.click()} style={{ height: '200px', background: '#0a0a0a', border: '2px dashed #222', borderRadius: '25px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          {imagePreview ? <img src={imagePreview} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <Upload opacity={0.3} />}
        ...
        <div style={{ background: '#0a0a0a', padding: '25px', borderRadius: '30px', border: '1px solid #1a1a1a', display: 'grid', gap: '15px' }}>
          <input placeholder="اسم المنتج" style={{ background: '#050505', border: '1px solid #222', padding: '15px', borderRadius: '15px', color: '#fff', outline: 'none' }} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
          ...
          <button onClick={handleSave} disabled={saving} style={{ background: 'linear-gradient(45deg, #d4af37, #fbf5b7)', color: '#000', padding: '18px', borderRadius: '15px', fontWeight: 900, border: 'none', marginTop: '10px' }}>
